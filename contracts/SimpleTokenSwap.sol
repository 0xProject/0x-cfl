...
contract SimpleTokenSwap {
    ...
    // See https://github.com/0xProject/0x-settler/tree/master?tab=readme-ov-file#allowanceholder-addresses
    ...
    // Swaps ERC20->ERC20 tokens held by this contract using a 0x Swap API quote.
    function fillQuote(
        // The `sellToken` field from the API response.
        IERC20 sellToken,
        // The `buyToken` field from the API response.
        IERC20 buyToken,
        // The `spender` field from the API response.
        address spender,
    ...
    {
        // Checks that the swapTarget is actually the address of AllowanceHolder contract
        require(swapTarget == allowanceHolder, "Target not AllowanceHolder");

        // Track our balance of the buyToken to determine how much we've bought.
        uint256 boughtAmount = buyToken.balanceOf(address(this));

        // If spender is not the zero address, give `spender` an infinite allowance to spend this contract's `sellToken`.
        if (spender != address(0)) {
            // Note that for some tokens (e.g., USDT, KNC), you must first reset any existing allowance to 0 before being able to update it.
